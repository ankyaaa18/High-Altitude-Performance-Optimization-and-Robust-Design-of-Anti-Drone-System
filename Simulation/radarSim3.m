...
droneMarker = polarscatter(radarAx,...
 ...
 'filled');
%% ============================================================
% 3D DRONE FLIGHT PANEL
% =============================================================

flightPanel = uipanel(fig, ...
 'Title','3D DRONE FLIGHT VIEW', ...
 'Position',[20 350 600 350], ...
 'BackgroundColor',[0.07 0.09 0.13], ...
 'ForegroundColor',[0.2 0.9 0.8], ...
 'FontSize',13, ...
...
flightAx = uiaxes(flightPanel, ...
 'Position',[40 35 520 270]);

flightAx.BackgroundColor = [0.03 0.04 0.06];

xlabel(flightAx,'X Position (m)');
ylabel(flightAx,'Y Position (m)');
zlabel(flightAx,'Altitude (m)');

title(flightAx,'LIVE 3D DRONE TRAJECTORY');

grid(flightAx,'on');

xlim(flightAx,[0 3000]);
ylim(flightAx,[-1000 2500]);
zlim(flightAx,[0 8000]);
...
% Ground sensor
plot3(flightAx,0,0,0,...
 'o',...
 'MarkerSize',10,...
 'LineWidth',2);

% Drone marker
drone3D = plot3(flightAx,...
 NaN,NaN,NaN,...
 'o',...
 'MarkerSize',12,...
 'LineWidth',2);
...
% Forward motion
droneX = 500 + 20*timeValue;

% Curved lateral motion
droneY = 1000 + ...
 500*sin(0.08*timeValue);

% Altitude controlled by slider
droneZ = altitude;
%% ============================================================
% STORE DRONE TRAJECTORY
% =============================================================

xHistory(end+1) = droneX;
yHistory(end+1) = droneY;
zHistory(end+1) = droneZ;

%% Update drone position

drone3D.XData = droneX;
drone3D.YData = droneY;
drone3D.ZData = droneZ;

%% Update flight path

trajectory3D.XData = xHistory;
trajectory3D.YData = yHistory;
trajectory3D.ZData = zHistory;
 %% Range
